...
void WindPlugin3::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
{
  ...
  this->_realpos_pub = _nh.advertise<nav_msgs::Odometry>("/drone1/realilinkpose", 5, true);
  ...
  this->pruebitaPub = _nh.advertise<nav_msgs::Odometry>("pruebita",10);
...
void WindPlugin3::OnUpdate()
{
  ...
  for (auto const &model : models)
  {
    ...
    for (auto const &link : links)
    {
      ...
      std::string dasname=link->GetName();
      ...
      if (dasname=="iris_demo::iris::base_link")
      {
      ...
      realpos_.child_frame_id = dasname;
      realpos_.header.frame_id = nombre;

      //ROS_WARN("ReLativeLinearSpeed = [%f,%f] ",linear.X(), linear.Y());
      if (dasname=="iris_demo::iris::base_link"||dasname=="drone1::iris_demo::iris::base_link")
      {
        ...
        if (i%100==0)
        {
          ...
          if (nombre=="drone1")
          {
            this->_realpos_pub.publish(realpos_);
          }
...
void conversion_cb(const nav_msgs::Odometry::ConstPtr& conversion)
{
    double x = conversion ->pose.pose.position.x;
    double y = conversion ->pose.pose.position.y;
    //ROS_INFO("pruebita_x: %f, pruebita_y: %f", x, y);
    _conversionPub1.publish(conversion);  
void pruebita_cb(const nav_msgs::Odometry::ConstPtr& pruebita){
  //double x = pruebita ->pose.pose.position.x;
  //double y = pruebita ->pose.pose.position.y;
  //ROS_INFO("pruebita_x: %f, pruebita_y: %f", x, y);
  this->pruebitaPub.publish(pruebita);  
