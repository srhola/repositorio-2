...
actitud2=[0,0,0,0]
actitud3=[0,0,0,0]
#no se puede importar mensajes de tipo vehicle debido a que se debe cargar un vehiculo dentro del script
...
def angulo_en_configuracion_real(pos1,pos2,indice,yaw1,desfase):
    modulo=math.sqrt((pos1[0]-pos2[0])**2+(pos1[1]-pos2[1])**2)
    unitario=[(pos2[0]-pos1[0])/modulo,(pos2[1]-pos1[1])/modulo]

    yaw_config=arcotangente2(unitario[1],unitario[0])

    '''
    #entre el III y IV cuadrante hay diferencia de signos  
    if yaw1*yaw_config<0 and abs(yaw1-yaw_config)>math.pi:
        if (yaw1)<0 and yaw_config>0:
            yaw1_aux=(yaw1)+2*math.pi
            yaw_config_aux=yaw_config
        elif yaw_config<0 and (yaw1)>0:
            yaw_config_aux=yaw_config+2*math.pi
            yaw1_aux=(yaw1)
    elif yaw1*yaw_config>0 and abs(yaw1+desfase-yaw_config)>1.5*math.pi:
        yaw1_aux=(yaw1)
        yaw_config_aux=yaw_config-math.pi*2
    else:
        yaw1_aux=(yaw1)
        yaw_config_aux=yaw_config
    ...
    yaw1_aux=(yaw1)
    #print('yaw',indice,yaw1,yaw_config-desfase)
    yl=yaw_config-desfase

    #print('yaw',indice,yaw1_aux,yaw_config_aux)
    return yl, yaw1_aux


def angulo_en_configuracion_real2(pos1,pos2,indice,yaw1,desfase):
    modulo=math.sqrt((pos1[0]-pos2[0])**2+(pos1[1]-pos2[1])**2)
    unitario=[(pos2[0]-pos1[0])/modulo,(pos2[1]-pos1[1])/modulo]
    #print(-unitario[1], unitario[0])
    yaw_config=arcotangente2(-unitario[1],unitario[0])
    ...
    '''
    yl=yaw_config-math.pi/2

    yaw1_aux=(yaw1)
    #print('yaw',indice,yaw1,-yl)
...
#The original code came from: https://stackoverflow.com/questions/35145555/python-real-time-plotting-ros-data
...
#    def odom_callback(self, msg1, msg2,msg3,odom1,odom2,odom3):
    def odom_callback(self,odom1,odom2,odom3):
        ...
        desfase2=3*math.pi/2
        desfase3=math.pi/2
        [yaw_configuracion12,yaw1_aux]=angulo_en_configuracion_real2(posicion1,posicion2,12,yaw1,desfase2)
        [yaw_configuracion13,yaw1_aux]=angulo_en_configuracion_real(posicion1,posicion3,13,yaw1,desfase3)
        yaw_de_comparacion12= yaw1_aux-yaw_configuracion12+desfase2*0
        yaw_de_comparacion13= yaw1_aux-yaw_configuracion13+desfase3*0
        #print('yaw',yaw1,yaw2,yaw3)
        
        #print('yaw_config',yaw1,'1-2',yaw_de_comparacion12,'1-3',yaw_de_comparacion13)
        #print('yaw1',yaw1,'yawn1',yawn1,  'yaw2',yaw2,'yawn2',yawn2,  'yaw3',yaw3,'yawn3',yawn3)
        #print('actitud1',actitud1[2],'actitud2',actitud2[2],'actitud3',actitud3[2])
        ...
        yaw_error1=yaw1-actitud1[2]*-1
        yaw_error2=yaw2-actitud2[2]*-1
        yaw_error3=yaw3-actitud3[2]*-1



        error_x=posicion1[0]-lat1
        error_y=posicion1[1]-lon1*-1
        error_z=posicion1[2]-alt_rel2*-1
       ...
       # self.y_data0.append(yawn1)
        ...
        #erroryaw13 corresponde a la diferencia entre el angulo de yaw del drone1 obtenido del gps con el angulo del drone3 en la configuracion
        #self.y_data1.append(erroryaw12)
        #self.y_data2.append(erroryaw13)
        
        #self.y_data2.append(rapidez)
        
        #self.y_data0.append(yaw_error1)
        #self.y_data1.append(yaw_error2)
        #self.y_data2.append(yaw_error3)
        if abs(yaw_de_comparacion12)>6:
            yaw_de_comparacion12=0
        else:
            yaw_de_comparacion12=yaw_de_comparacion12


        self.y_data0.append(yaw1_aux)
        self.y_data1.append(yaw_de_comparacion12)
        self.y_data2.append(yaw_de_comparacion13)
