...
def subscriber():
    rospy.init_node('listenerdrone3', anonymous=False)
    ...
    counter1=0
    counter2=0
    [hlat3,hlon3,halt_rel3]=lat_lon_alt(vehicle.home_location)
    while counter1<(180): 
        [lat,lon,alt_rel]=lat_lon_alt(vehicle.location.local_frame)
        [hlat1,hlon1,halt_rel1]=lat_lon_alt(home1)   
        [glat3,glon3,galt_rel3]=lat_lon_alt(vehicle.location.global_relative_frame)
        ...
        [vyaw_E,vyaw_N,erroryaw]=control_yaw(yawn1,lat1,lon1,lat,lon,20)
        [vector_N, vector_E, modulo]=mantener_distancia(20,lat1,lon1,lat,lon-5)
        #print(vector_N,vector_E,"diferencia1y2",diferencia, modulo)
        print("diferencia1y3",modulo)
...
def velocidades(dE,dN,yaw,separacion):
    ...
    if 5<(cte*d*d):
        ...
        vc_E=-5*vcu_E
    return vc_N, vc_E
def control_yaw(yaw1,lat1,lon1,lat3,lon3,r):
    yaw3=0
    conrespecto=-math.pi/2
    modulo=math.sqrt((lat3-lat1)**2+(lon3-lon1)**2)
    u_N=(lat3-lat1)/modulo
    u_E=(lon3-lon1)/modulo

    #yaw3=math.pi-arcotangente(u_N,u_E)
    yaw3=arcotangente2(u_E,u_N)
    #entre el III y IV cuadrante hay disferencia de signos  
    if yaw1*yaw3<0 and abs(yaw1-yaw3)>math.pi:
        if (yaw1)<0 and yaw3>0:
            yaw1_aux=(yaw1)+2*math.pi
            yaw3_aux=yaw3
        elif yaw3<0 and (yaw1)>0:
            yaw3_aux=yaw3+2*math.pi
            yaw1_aux=(yaw1)
    elif yaw1*yaw3>0 and abs(yaw1+conrespecto-yaw3)>1.5*math.pi:
        yaw1_aux=(yaw1)
        yaw3_aux=yaw3-math.pi*2
    else:
        yaw1_aux=(yaw1)
        yaw3_aux=yaw3

    error=abs(yaw1+conrespecto-yaw3)
    error2_puro=(yaw1_aux+conrespecto-yaw3_aux)
    error2=abs(yaw1_aux+conrespecto-yaw3_aux)
    #print('error',error,'yaw1',yaw1,'yaw3',yaw3)
    print('error',error,'yaw1',yaw1,'yaw3',yaw3,'error2',error2,'yaw1_aux',yaw1_aux,'yaw3_aux',yaw3_aux)
    if error2<0.5:
        velocidad_rotacion=error2*2
    else:
        velocidad_rotacion=3
    r=20
    Periodo=2*math.pi*r/velocidad_rotacion
    #w=abs(yaw1-yaw3)/Periodo
    w=2*math.pi/Periodo 
    if (yaw1_aux+conrespecto-yaw3_aux)>0:
        vrot_x=+w*r*math.cos(yaw3)
        vrot_y=-w*r*math.sin(yaw3)
...
def connectMyCopter2():
    ...
    connection_string= '127.0.0.1:14571'
    vehicle = connect(connection_string,wait_ready=True)
    return vehicle
...
def odometria(x,y,z,roll,pitch,yaw,vx,vy,vz,v_angx,v_angy,v_angz):
    odom_pub = rospy.Publisher("drone3/odom", Odometry, queue_size=50)
