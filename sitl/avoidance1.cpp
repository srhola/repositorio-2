...
#include <sstream>
ros::Publisher _servant1_pub;

//void callback4 (const std_msgs::Float64::ConstPtr& msg ){
	//double x = msg ->pose.pose.position.x;
	//double y = msg ->pose.pose.position.y;
	//ROS_INFO("x: %f, y: %f", x, y);
	//printf("Rin the thigh queen!\n");
	...
	for (int i = 1; i < current_2D_scan.ranges.size(); i++)
	  {
	  	float d0=3;
	  	float k = 0.5;
	  	std_msgs::Float64 miporin;
	  	//miporin.data=d0;
	  	//printf("current_2D_scan.ranges\n",current_2D_scan.ranges);

	  	if (current_2D_scan.ranges[i] <102 && current_2D_scan.ranges[i]>0.35)
	  	{
	  		//ROS_INFO("range: %f angle: %f ", current_2D_scan.ranges[i], current_2D_scan.angle_increment*i );
	  		miporin.data=current_2D_scan.ranges[i];

	  		_servant1_pub.publish(miporin);
	  	}
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle n;
	_servant1_pub = n.advertise<std_msgs::Float64>("/distancia_lidar", 5, true);
